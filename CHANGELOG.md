...
- **Sous-commande CLI `dest` — gérer le routage sans IA** : trois commandes pour construire et maintenir le fichier de destinations sans passer par la fenêtre de revue. `dest list` affiche les destinations existantes (chemin, règles, note) et signale les anomalies (double `default`, chemin vide). `dest add <chemin> [--domain/--from/--subject/--account/--note/--default]` ajoute une destination — un chemin nu devient une simple option de classement, les drapeaux ajoutent des règles de routage automatique (plusieurs drapeaux = plusieurs règles en un appel) ; validation anti-traversal et garde contre un second `default`. `dest suggest` scanne le dossier par défaut sous `notes_dir`, regroupe les emails par domaine d'expéditeur (frontmatter `from:`), écarte les domaines déjà couverts, et propose interactivement une destination par domaine (le scan exclut les dossiers `.`/`_`, ne suit pas les symlinks, et est borné en profondeur). Nouveaux modules `destinations.rs` (stockage YAML) et `dest_cmd.rs` (CLI). 25 tests ajoutés.
...
- **Format des destinations : `destinations.txt` → `destinations.yaml`** : le routage passe d'un format texte maison à du YAML structuré, cohérent avec `accounts.yaml`/`settings.yaml`. Chaque entrée porte `path`, des `rules` typées (`domain`/`from`/`subject`/`account`), une `note` optionnelle (remplace les commentaires libres) et un drapeau `default`. Les règles sont sérialisées en maps à clé unique (`- domain: ubs.ch`) via `serde_yaml::with::singleton_map_recursive`. **Migration automatique et non destructive** : au premier chargement, un `destinations.txt` existant est converti vers son voisin `.yaml` (le `.txt` est laissé en place). La migration est centralisée dans `route::destinations_path()` pour que tous les points d'entrée (export, CLI `dest`, tray) en bénéficient. `parse_destinations` est conservé uniquement comme lecteur legacy de migration ; `upsert_rule` délègue désormais au stockage YAML (signature inchangée pour le tray).
- **Nom des pièces jointes plus lisible** : le schéma passe de `<préfixe-email>__<hash8>_<nom>` à **`<date>_<nom-original>`** (ex. `2026-06-25_Facture.pdf`). Le hash cryptique et le long préfixe disparaissent ; le nom d'origine est immédiatement lisible. Les collisions intra-dossier restent gérées par suffixe numérique (`_2`, `_3`).
- **`move_email` — anti-collision en destination** : comme le nom de PJ ne contient plus de partie propre à l'email, deux emails distincts routés vers le **même dossier** peuvent porter le même nom de fichier (ex. `2026-06-25_image001.png`). `move_email` suffixe désormais le nom en cas de collision dans le dossier cible (nouveau helper `unique_name_in`) et **met à jour les liens** (frontmatter `attachments:` + lien Markdown du corps) du `.md` déplacé, évitant tout écrasement silencieux. 1 test ajouté.
